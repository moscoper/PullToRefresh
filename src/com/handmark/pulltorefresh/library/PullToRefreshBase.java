...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
  ...
  public void setHeadBgImage(ImageView headBg){
    this.headBg = headBg;
    if (headBg != null){
      headBgH = headBg.getLayoutParams().height;
      headBgLastH=headBgH;
    }
  }
  ...
  @Override public final boolean demo() {
  ...
  @Override public final Mode getCurrentMode() {
    return mCurrentMode;
  }

  @Override public final boolean getFilterTouchEvents() {
    return mFilterTouchEvents;
  }

  @Override public final ILoadingLayout getLoadingLayoutProxy() {
  ...
  @Override public final Mode getMode() {
    return mMode;
  }

  @Override public final T getRefreshableView() {
    return mRefreshableView;
  }

  @Override public final boolean getShowViewWhileRefreshing() {
  ...
  @Override public final boolean isPullToRefreshEnabled() {
    return mMode.permitsPullToRefresh();
  }

  @Override public final boolean isPullToRefreshOverScrollEnabled() {
    return VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD
        && mOverScrollEnabled
        && OverscrollHelper.isAndroidOverScrollEnabled(mRefreshableView);
  }

  @Override public final boolean isRefreshing() {
    return mState == State.REFRESHING || mState == State.MANUAL_REFRESHING;
  }

  @Override public final boolean isScrollingWhileRefreshingEnabled() {
  ...
  @Override public final void onRefreshComplete() {
    if (isRefreshing()) {
      setState(State.RESET);
    }
  }

  public void reSetHeadSize(float height){
    if (headBg != null && height > 0){
      ViewGroup.LayoutParams layoutParams = headBg.getLayoutParams();

      if(headBgLastH>headBgH){
        layoutParams.height = (int) (headBgLastH+height);
      }else {
        layoutParams.height = (int) (headBgH+height);
      }
    ...
    switch (event.getAction()) {
      case MotionEvent.ACTION_MOVE: {
        if (mIsBeingDragged) {
          ...
          float dis = mLastMotionY - mInitialMotionY;
          pullEvent();
          if(dis>0){
            float newScrollValue = Math.round(Math.max(dis, 0) / FRICTION);
            reSetHeadSize(newScrollValue);
          }
      ...
      case MotionEvent.ACTION_UP: {
        if (mIsBeingDragged) {
          mIsBeingDragged = false;
          if (headBg != null){
            headBgLastH = headBg.getLayoutParams().height;
          }
  ...
  @Override public final void setFilterTouchEvents(boolean filterEvents) {
  ...
  @Override public final void setMode(Mode mode) {
    if (mode != mMode) {
      if (DEBUG) {
  ...
  @Override public final void setOnRefreshListener(OnRefreshListener<T> listener) {
    mOnRefreshListener = listener;
    mOnRefreshListener2 = null;
  }

  @Override public final void setOnRefreshListener(OnRefreshListener2<T> listener) {
  ...
  @Override public final void setPullToRefreshOverScrollEnabled(boolean enabled) {
    mOverScrollEnabled = enabled;
  }

  @Override public final void setRefreshing() {
    setRefreshing(true);
  }

  @Override public final void setRefreshing(boolean doScroll) {
    if (!isRefreshing()) {
  ...
  @Override public final void setShowViewWhileRefreshing(boolean showView) {
  ...
  @Override protected final void onRestoreInstanceState(Parcelable state) {
  ...
  @Override protected final Parcelable onSaveInstanceState() {
  ...
  @Override protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
    if (DEBUG) {
  ...
  protected final void refreshLoadingViewsSize() {
    ...
    if (DEBUG) {
      Log.d(LOG_TAG,
          String.format("Setting Padding. L: %d, T: %d, R: %d, B: %d", pLeft, pTop, pRight,
              pBottom));
    }
  ...
  final class SmoothScrollRunnable implements Runnable {
    ...
    public SmoothScrollRunnable(int fromY, int toY, long duration,
      ...
      mListener = listener;
      if (headBg != null){
        oldHeight = headBg.getLayoutParams().height;
        headBgLastH=oldHeight;
        //newHeigth =
      }
    }
    ...
    @Override public void run() {
      ...
      if (mStartTime == -1) {
        ...
        if(mScrollFromY<0){
          if (headBg != null){
            //  final int headDis = Math.round(
            //      (oldHeight - headBgH) * mInterpolator.getInterpolation(normalizedTime / 1000f));
            ViewGroup.LayoutParams layoutParams = headBg.getLayoutParams();
            if (layoutParams.height >headBgH){
              layoutParams.height = oldHeight + deltaY;
              headBg.setLayoutParams(layoutParams);
            }else{
              layoutParams.height = headBgH;
              headBg.setLayoutParams(layoutParams);
            }
            headBgLastH=layoutParams.height;
            if(!isRefreshing()){//这里重置一下
              if(0==mCurrentY){
                layoutParams.height = headBgH;
                headBg.setLayoutParams(layoutParams);
                headBgLastH=headBgH;
